...
def main():
    ...
    while (cap.isOpened()):
        ...
        cnts , fds= cv2.findContours(thresh.copy(), cv2.RETR_TREE, cv2.CHAIN_APPROX_NONE)[-2:]
        center = None
        if len(cnts) >= 1:
            ...
            if len(pts) != []:
                ...
                blackboard_cnts ,sifha= cv2.findContours(thresh1.copy(), cv2.RETR_TREE, cv2.CHAIN_APPROX_NONE)[-2:]
